...
class NotificationCenter {
  ...
  private _createNotification = (message: string) => {
    ...
    notification.innerHTML = `
      <div class="notification ${"flex items-center gap-2 bg-(--uiBgLightTheme) text-(textLightTheme) dark:bg-(--uiBgDarkTheme) dark:text-(--textDarkTheme) py-2 px-4 rounded-(--uiRadius) shadow cursor-pointer"}">
