...
class NotificationCenter {
  ...
  private _createNotification = (message: string) => {
    ...
    notification.innerHTML = `
      <div class="notification ${"flex items-center gap-2 bg-(--ui_bg_lt) text-(--text_lt) dark:bg-(--ui_bg_dt) dark:text-(--text_dt) py-2 px-4 rounded-(--ui_radius) shadow cursor-pointer text-sm"}">
